...
class EditorPicker extends MusicBeatSubstate {
    ...
    public override function create() {
        ...
        bg = new FlxSprite().loadGraphic(Paths.image('assets/menus/menuBGBlue'));
        bg.scrollFactor.set();
    ...
    public override function update(elapsed:Float) {
        ...
        if (controls.ACCEPT || !controls.touchC && FlxG.mouse.justReleased) {
            if (options[curSelected].state != null) {
				...
				// replace url to a message if u want idk
                // CoolUtil.showMessage("This option is not implemented yet.");
            }
        }
        if (controls.BACK)
            close();
...
class EditorPickerOption extends FlxTypedSpriteGroup<FlxSprite> {
    ...
    public function new(name:String, iconID:Int, height:Float) {
        ...
        iconSpr.loadGraphic(Paths.image('assets/editors/icons'), true, 128, 128);
