...
class DialogueCutscene extends Cutscene {
    ...
    public function new(dialoguePath:String, callback:Void->Void) {
        ...
        dialogueScript = Script.create("assets/data/scripts/" + Path.withoutExtension(dialoguePath) + ".hx");
    ...
    public override function create() {
        ...
        try {
            ...
            event.dialogueData = new Access(Xml.parse(Assets.getText("assets/data/dialogues/" + dialoguePath)).firstElement());
            ...
            for(node in dialogueData.nodes.line) {
                ...
                var line:DialogueLine = {
                    ...
                    changeMusic: node.has.changeMusic ? FlxG.sound.load("assets/music/" + node.att.changeMusic + ".ogg", volume, true) : null,
                    playSound: node.has.playSound ? FlxG.sound.load("assets/sounds/" + node.att.playSound + ".ogg") : null,
                    nextSound: node.has.nextSound ? FlxG.sound.load("assets/sounds/" + node.att.nextSound + ".ogg") : null,
                    textSound: null
                };

                if(node.has.textSound) line.textSound = FlxG.sound.load("assets/sounds/" + node.att.textSound + ".ogg");
                else if(!useDef) {
                    var char:DialogueCharacter = charMap[line.char];
                    if(char != null && char.charData != null && char.charData.has.textSound)
                        line.textSound = FlxG.sound.load("assets/sounds/" + char.charData.att.textSound + ".ogg");
                }
